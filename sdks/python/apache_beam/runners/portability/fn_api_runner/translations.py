...
class TransformContext(object):
  ...
  def add_data_channel_coder(self, pcoll_id, is_input=False):
    ...
    windowed_coder_id = self.add_or_get_coder_id(proto, pcoll.coder_id + '_windowed')
  ...
  for stage in stages:
    for transform in stage.transforms:
      if transform.spec.urn in (bundle_processor.DATA_INPUT_URN,
        ...
        pipeline_context.add_data_channel_coder(sdk_pcoll_id, transform.spec.urn == bundle_processor.DATA_INPUT_URN)
